function initMegaNavDirectionalHover() {
  const navTimeline = (options) => {
    const timeline = gsap.timeline(options);
    if (matchMedia('(prefers-reduced-motion: reduce)').matches) timeline.timeScale(100);
    return timeline;
  };
  const DUR = {
    bgMorph: 0.4,
  ...
  // Osmo Burger Menu Button: supplied CustomEase and staged line movement.
  gsap.registerPlugin(CustomEase);
  CustomEase.create("button-ease", "0.5, 0.05, 0.05, 0.99");
  function animateBurger(toX) {
    burger.setAttribute("data-menu-button", toX ? "close" : "burger");
    const tl = navTimeline({ defaults: { overwrite: "auto", ease: "button-ease", duration: 0.3 } });
    if (toX) {
      tl.to(lineMid, { scaleX: 0, opacity: 0 })
        .to(lineTop, { x: "-1.3em", opacity: 0 }, "<")
        .to(lineBot, { x: "1.3em", opacity: 0 }, "<")
        .to([lineTop, lineBot], { opacity: 0, duration: 0.1 }, "<+=0.2")
        .set(lineTop, { rotate: -135, y: "-1.3em", scaleX: 0.9 })
        .set(lineBot, { rotate: 135, y: "-1.4em", scaleX: 0.9 }, "<")
        .to(lineTop, { opacity: 1, x: "0em", y: "0.5em" })
        .to(lineBot, { opacity: 1, x: "0em", y: "-0.25em" }, "<+=0.1");
    } else {
      tl.to([lineTop, lineMid, lineBot], {
  ...
  function openMobilePanel(panelName) {
    ...
    resetToggles();
    getToggle(panelName)?.setAttribute("aria-expanded", "true");
  ...
  function closeMobilePanel() {
    ...
    const tl = navTimeline({
      onComplete() { state.mobilePanelActive = null; state.mobilePanelTl = null; resetToggles(); },
    });
    state.mobilePanelTl = tl;
  ...
  function handleResize() {
    ...
    resizeTimer = setTimeout(() => {
      ...
      if (was && !state.isMobile) {
        ...
        gsap.set([lineTop, lineMid, lineBot], { rotation: 0, x: 0, y: 0, scaleX: 1, autoAlpha: 1 });
        burger.setAttribute("data-menu-button", "burger");
  ...
  // Close mobile navigation on an in-page link selection.
  menuWrap.querySelectorAll('a').forEach(link => link.addEventListener('click', () => {
    clearTimers();
    if (state.isMobile && state.mobileMenuOpen) closeMobileMenu();
    else if (state.isOpen) closeDropdown();
  }));
  // Respect reduced motion while preserving the supplied GSAP timeline system.
